/*
  ...
  Versões:
    ...
    v7 - Comandos RF - OK
    v8 - Comandos para janelas e sensor de chuva
*/
...
#define   in1  31      // entrada 1 do motor de passo
#define   in2  33      // entrada 2 do motor de passo
#define   in3  35      // entrada 3 do motor de passo
#define   in4  37      // entrada 4 do motor de passo
...
int StatusSensorChuva = 0;
int sensor_chuva= A1; // analog sensor input pin 0
int status_janela = 0; // 1 - aberta, 0 - fechada

void setup()
{
  myStepper.setSpeed(5); // velocidade 100 rpm:
  radio.begin();
...
void loop()
{
  int dadoEnvio0 = 0; // Sinal desligar lâmpada
  int dadoEnvio1 = 1; // Sinal ligar lâmpada

  int valorPIR = digitalRead(pinSensorPIR);
  int valorLum = analogRead(pinSensorLum); 

  int leitor_chuva = analogRead(sensor_chuva);
  delay(10);
  //Serial.println(leitor_chuva);
  ...
  {
    // Ligar lâmpada real
    radio.write( &dadoEnvio1, sizeof(int) );
    delay(1000);
    radio.write( &dadoEnvio1, sizeof(int) );
  }
  if (comando == 'x')
  {
    // Desligar lâmpada real
    radio.write( &dadoEnvio0, sizeof(int) );
    delay(1000);
    radio.write( &dadoEnvio0, sizeof(int) );
  }

  if (status_janela == 0 && comando == 'Y' && leitor_chuva > 800)
  {
   // abrir toda
  delay(10);
  myStepper.step(600);
  ...
  if (comando == 'K')
  {
    StatusSensorChuva = 1;   // Vai ativar sensor de chuva
  }  
  if (comando == 'k')
  {
    StatusSensorChuva = 0;   // Vai desativar sensor de chuva
  }
  if ((valorLum>900) && (StatusSensorLum == 1))   
  ...
  if (status_janela == 1 && StatusSensorChuva == 1 && leitor_chuva < 800) { 
  myStepper.step(-600);
  status_janela = 0;
  }
}
